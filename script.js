...
function renderUserQuizzes(quizzesUser) {
    document.querySelector(".userQuizzesEmpty").classList.add("hidden");
    ...
    for (let i = 0; i < quizzesUser.length; i++) {
        board.innerHTML += `
            <div onclick="getClickedQuizz(this)" class="Quizz">
                <img src="${quizzesUser[i].image}" alt="quizz image">
                <div class="img_bgd"></div>
                <div>
                    <h1>${quizzesUser[i].title}</h1>
                    <span class="id hidden">${quizzesUser[i].id}</span>
                </div>
            </div>
    `;
    }
